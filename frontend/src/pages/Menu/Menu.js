...
const Menu = () => {
  ...
  const [menuMeta, setMenuMeta] = useState({ usingDefaultMenu: false, restaurantName: '' });
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        const { data } = await API.get('/restaurant/public');
        setRestaurants(data);
        if (data.length > 0) {
          setSelectedRestaurant((prev) => prev || data[0]._id);
        }
      } catch {
        setRestaurants([]);
      }
  ...
  useEffect(() => {
    if (!selectedRestaurant) {
      setFoods(demoFoods);
      setMenuMeta({ usingDefaultMenu: true, restaurantName: 'Demo Restaurant' });
      setLoading(false);
      return;
    }
    ...
    const fetchFoods = async () => {
      try {
        setLoading(true);
        const { data } = await API.get(`/restaurant/${selectedRestaurant}/menu`);
        const foodsWithRestaurant = (data.foods || []).map((item) => ({
          ...item,
          restaurantId: selectedRestaurant,
          restaurantName: data.restaurant?.restaurantDetails?.restaurantName || data.restaurant?.name || 'Restaurant',
        }));
        setFoods(foodsWithRestaurant.length > 0 ? foodsWithRestaurant : demoFoods);
        setMenuMeta({
          usingDefaultMenu: data.usingDefaultMenu,
          restaurantName: data.restaurant?.restaurantDetails?.restaurantName || data.restaurant?.name || 'Restaurant',
        });
      } catch {
        setFoods(demoFoods);
  ...
  const handleAdd = (food) => {
    const existingItems = Object.values(cartItems);
    const currentRestaurantId = existingItems[0]?.restaurantId;

    if (currentRestaurantId && food.restaurantId && currentRestaurantId !== food.restaurantId) {
      toast.error('Your cart already has items from another restaurant. Please clear cart first.');
      return;
    }
  ...
  return (
    <div className="menu-page">
        ...
        <div className="menu-actions">
          ...
          >
            {restaurants.length === 0 ? (
              <option value="">Demo Restaurant</option>
            ) : (
      ...
      <div className="restaurant-meta">
        <strong>{menuMeta.restaurantName}</strong>
        {menuMeta.usingDefaultMenu && (
          <span className="default-menu-chip">Using default menu template</span>
        )}
      </div>
